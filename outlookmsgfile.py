...
def load_message_stream(entry, is_top_level, doc):
    ...
    props = parse_properties(
        entry['__properties_version1.0'], is_top_level, entry, doc)
...
def process_attachment(msg, entry, doc):
    ...
    props = parse_properties(
        entry['__properties_version1.0'], False, entry, doc)

    # The attachment content...
    blob = props['ATTACH_DATA_BIN']

    # Get the filename and MIME type of the attachment.
    filename = props.get("ATTACH_LONG_FILENAME") or props.get(
        "ATTACH_FILENAME") or props.get("DISPLAY_NAME")
    ...
    if isinstance(blob, str):
        ...
        msg.add_attachment(
            blob,
            maintype=mime_type.split(
                "/", 1)[0], subtype=mime_type.split("/", 1)[-1],
            filename=filename)
...
def parse_properties(properties, is_top_level, container, doc):
    ...
    while i < len(stream):
        ...
        # Variable Length Properties.
            ...
            # unrecognized type
            print("unhandled property type", hex(
                property_type), file=sys.stderr)
            continue
...
class INTTIME(FixedLengthValueLoader):
    ...
    def load(value):
        ...
        value = reduce(lambda a, b: (a << 8)+b, reversed(value))
